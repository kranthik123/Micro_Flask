pipeline {
    ...
    stages {
        ...
        stage('Build'){steps{withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'DockerHubCreds', usernameVariable: 'USERNAME']]){sh "sudo docker build . -t flask_app:latest && sudo docker build . -t '${USERNAME}'/flask_app:latest"}}}
        ...
        stage('push-image'){steps{withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'DockerHubCreds', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD']]){sh "docker login -u '${USERNAME}' -p '${PASSWORD}'"}sh 'docker push '${USERNAME}'/flask_app:latest'}}}
